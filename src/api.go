...
/*
	borrowed from github.com/alireza0/x-ui/xray/api.go
*/
import (
	"context"
	"encoding/json"
	...
	"liberator-xray-commander/src/common"
	"regexp"
	"time"
)
...
func (x *XrayAPI) Init(apiHost string) (err error) {
	if apiHost == "" {
		return common.NewError("xray api host isn't defined")
	}
	x.grpcClient, err = grpc.Dial(apiHost, grpc.WithInsecure())
	if err != nil {
		return err
	}
	x.isConnected = true
...
func (x *XrayAPI) AddInbound(inbound []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to unmarshal inbound: %v", err)
	}
	config, err := conf.Build()
	if err != nil {
		return fmt.Errorf("failed to build inbound Detur: %v", err)
	}
